...
# # # # # # # # # # # # # # # # # # # # 
GPU Example:
CUDA_VISIBLE_DEVICES=1 python HDMovienet_3D_4motionstates_withoverlap.py  myx5d.npy  4Dmrioutput.mat
CPU Example:
CUDA_VISIBLE_DEVICES=, python HDMovienet_3D_4motionstates_withoverlap.py  myx5d.npy  4Dmrioutput.mat
# # # # # # # # # # # # # # # # # # # # 
...
print(zIn.shape)
#
windowOffset = int(np.floor(NZvolume/2))
print('windowOffset: {}'.format(windowOffset))
# zIn = zIn[(NMx-1)+windowOffset:(NMx-1)+NM+windowOffset,(NMx-1)+windowOffset:(NMx-1)+NM+windowOffset,(NMx-1)+windowOffset:(NMx-1)+NM+windowOffset,:,:]
zIn = zIn[(NMx-1):(NMx-1)+NM,(NMx-1):(NMx-1)+NM,(NMx-1)+windowOffset:(NMx-1)+NM+windowOffset,:,:]
print(zIn.shape)
zIn = convertInputMAT256(zIn)
print(zIn.shape)
#
...
with torch.no_grad():
    if train_on_gpu:
        test_data = test_data_extra.cuda()
    tTt0 = time.time()
    outresults = model(test_data_extra)
    tTt = time.time()
#
outresults = gobackfrom3D(outresults)
#
...
#
print('Time Testing ({} slices, no saving in HD): {:.6f}sec'.format(outresults.size(dim=0), tTt-tTt0))
outresults_extra = outresults
del outresults

#######################################################################
######################### COMBINED ##################################
combinedResults = 0*outresults_extra
print(combinedResults.shape)
#
i = np.array([0,1])
k = 2
while k<=(combinedResults.size(dim=2)-1):
    i = np.concatenate( (i,np.array([k+0])) )
    i = np.concatenate( (i,np.array([k+1])) )
    i = np.concatenate( (i,np.array([k+2])) )
    i = np.concatenate( (i,np.array([k+3])) )
    k = k + NZvolume
print(i)
combinedResults[:,:,i,:] = outresults_ori[:,:,i,:]
#
i = np.array([ ])
k = 2
while k<=(combinedResults.size(dim=2)):
    i = np.concatenate( (i,np.array([k+0])) )
    i = np.concatenate( (i,np.array([k+1])) )
    i = np.concatenate( (i,np.array([k+2])) )
    i = np.concatenate( (i,np.array([k+3])) )
    k = k + NZvolume
i = i[i<(NM-windowOffset)]
print(i)
print(i+windowOffset)
combinedResults[:,:,i+windowOffset,:] = outresults_extra[:,:,i,:]
# # # # # # # # # 
combinedResults = combinedResults[:,:,0:NM,:]
print(combinedResults.shape)
...
var2save = {"test_pd_ori":outresults_ori.detach().numpy(),"test_pd_extra":outresults_extra.detach().numpy(),"test_pd_combined":combinedResults.detach().numpy()}
